...
function indexIntoObject(baseObject, item, field, comparisonItem) {
    ...
    if (Array.isArray(nextFields) && nextFields.length) {
        ...
        indexIntoObject(baseObject[value], item, nextFields, comparisonItem);
    }
    else {
        ensureKeyOnObject(baseObject, value, []);
...
exports.indexListByFields = function () {
    ...
    return function (list, processor) {
        ...
        for (var i = 0, len = list.length; i < len; i++) {
            var item = list[i];
            var compareItem = typeof processor === 'undefined' ? list[i] : processor(item);
...
exports.memoizeIndexedArray = function (fn) {
    ...
    return function () {
        ...
        if (lastResult) {
            ...
            var fields = Object.keys(nextResult);
            var nextIndexed = fields.reduce(function (state, field) {
                var innerChanged = false;
                var fieldKeys = Object.keys(nextResult[field]);
                var nextValueIndexedList = fieldKeys.reduce(function (acc, value) {
